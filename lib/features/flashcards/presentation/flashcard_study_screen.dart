...
class _FlashcardStudyScreenState extends ConsumerState<FlashcardStudyScreen>
    ...
    try {
      ...
      if (audioUrl.isEmpty) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text(context.translate('no_audio_for_card'))),
        );
        return;
      }
  ...
  Widget build(BuildContext context) {
    ...
    final hasAudio =
        currentAudioUrl != null && currentAudioUrl.trim().isNotEmpty;

    final totalCount = sessionState.totalCount;
    final currentCount = totalCount == 0
        ? 0
        : math.min(sessionState.currentIndex + 1, totalCount);
                                ...
                                Expanded(
                                  child: Builder(
                                    builder: (context) {
                                      ...
                                      final originalBackText = card.backText
                                          ?.trim();
                                      final hasBackText =
                                          originalBackText?.isNotEmpty == true;
                                      final fallbackText = hasBackText
                                          ? originalBackText!
                                          : context.translate(
                                              'no_back_text_yet',
                                            );
                                      ...
                                      final cacheKey = llmCacheKey(
                                        targetLanguage,
                                        fallbackText,
                                      );
                                      final cache = ref.watch(
                                        llmTranslationCacheProvider,
                                      );
                                      ...
                                      final translation = ref.watch(
                                        llmTranslateProvider(request),
                                      );
                                      return translation.when(
                                        data: (output) {
                                          final translated =
                                              output.translatedTexts.isNotEmpty
                                              ? output
                                                    .translatedTexts
                                                    .values
                                                    .first
                                        ...
                                        loading: () =>
                                            buildScrollableText(fallbackText),
                                        error: (_, _) {
...
class _CompletionState extends StatelessWidget {
  const _CompletionState({required this.deckName, required this.onBack});
